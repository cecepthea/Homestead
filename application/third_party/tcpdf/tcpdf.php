...
// CUSTOM RESIZE CLASS
require_once(dirname(__FILE__).'/include/resize-class.php');
...
class TCPDF {
	 ...
	 * @protected
	 */
	protected $title = 'moli print';

	/**
	 * Document subject.
	 * @protected
	 */
	protected $subject = 'moli print pdf';

	/**
	 * Document author.
	 * @protected
	 */
	protected $author = 'Xingluo User';

	/**
	 * Document keywords.
	 * @protected
	 */
	protected $keywords = 'moli, PDF, print';

	/**
	 * Document creator.
	 * @protected
	 */
	protected $creator = 'Xingluo';
	...
	public function getDrawedImageWH($resize_mode, $file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array(), $stop_loop = false) {
		...
		$ori_fitbox = $fitbox;
		$ori_hidden = $hidden;
		$ori_fitonpage = $fitonpage;
		$ori_alt = $alt;
		$ori_altimgs = $altimgs;
		$ori_stop_loop = $stop_loop;
		...
		if ($file[0] === '@') {
			...
			$resizeObj = new \custom_resize($file);
 		    $resizeObj -> resizeImage('portrait'); // portrait 竖, landscape
		...
		if($stop_loop){
		    ...
		    if($w <= $c_w && $h <= $c_h){
				...
				$drawedWH['x'] = $origin_x + (($c_w - $w)/2);
				if($resize_mode == 'top'){
		            $drawedWH['y'] = $origin_y;
				}else{
		            $drawedWH['y'] = $origin_y + (($c_h - $h)/2);
				}
		    ...
		    if($common_deal){
        		$temp_res = $this->getDrawedImageWH($resize_mode, $ori_file ,$ori_x ,$ori_y ,$drawedWH['w'] ,$drawedWH['h'] ,$ori_type ,$ori_link ,$ori_align ,$ori_resize ,$ori_dpi ,$ori_palign ,$ori_ismask ,$ori_imgmask ,$ori_border ,$ori_fitbox ,$ori_hidden ,$ori_fitonpage ,$ori_alt ,$ori_altimgs ,true);

				$drawedWH['x'] = $origin_x + (($c_w - $temp_res['w'])/2);
				if($resize_mode == 'top'){
		        	$drawedWH['y'] = $origin_y;
				}else{
		        	$drawedWH['y'] = $origin_y + (($c_h - $temp_res['h'])/2);
				}
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		// header('Test-'.$file.': '.$imsize[0].' , '.$imsize[1]);
	...
	public function Custom_UtoP($unit){
		return $unit * ($this->imgscale * (150 / 25.4));
	}

	public function Custom_PtoU($px){
		return ($px / ($this->imgscale * (150 / 25.4)));
	}
